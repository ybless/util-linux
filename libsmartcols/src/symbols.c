...
/**
 * scols_symbols_set_alone_mark:
 * @sy: a pointer to a struct libscols_symbols instance
 * @str: a string which will mark alone nodes in group chart
 *
 * Returns: 0, a negative value in case of an error.
 *
 * Since: 2.34
 */
int scols_symbols_set_alone_mark(struct libscols_symbols *sy, const char *str)
{
	return strdup_to_struct_member(sy, mark_alone, str);
}

/**
 * scols_symbols_set_member_mark:
 * @sy: a pointer to a struct libscols_symbols instance
 * @str: a string which will mark group member
 *
 * Returns: 0, a negative value in case of an error.
 *
 * Since: 2.34
 */
int scols_symbols_set_member_mark(struct libscols_symbols *sy, const char *str)
{
	return strdup_to_struct_member(sy, mark_member, str);
}
...
struct libscols_symbols *scols_copy_symbols(const struct libscols_symbols *sy)
{
	...
	if (!rc)
		rc = scols_symbols_set_alone_mark(ret, sy->mark_alone);
	if (!rc)
		rc = scols_symbols_set_member_mark(ret, sy->mark_member);
	if (!rc)
